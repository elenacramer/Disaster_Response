import sys
import pandas as pd
from sqlalchemy import create_engine

def load_data(messages_filepath, categories_filepath):
    '''
    Function that loas two cvs files into pandas DataFrames and merges them into one.
    Inputs:
        messages_filepath, categories_filepath: filepath of the cvs files
    Output:
        df: merged DataFrames
    '''
    messages = pd.read_csv(messages_filepath)
    categories = pd.read_csv(categories_filepath)
    # merge datasets
    df = messages.merge(categories, how='outer', on='id')
    return df


def clean_data(df):
    '''
    Function that cleans i.e. prepares data for a model.
        - creates a dataframe 'categories' of the 36 individual category columns and renames the rows
        - drops the original categories column from `df`
        - concatenate the original dataframe with the new `categories` dataframe
        - drops duplicates
    Input:
        df: DataFrame
    Output: 
        df: DataFrame cleaned with the above mentioned steps
    '''
    # create a dataframe of the 36 individual category columns
    categories = df['categories'].str.split(pat=';', expand=True)
    # select the first row of the categories dataframe
    row = categories.iloc[0]
    # use this row to extract a list of new column names for categories.    
    category_colnames = row.apply(lambda x: x[:-2]).tolist()
    # rename the columns of `categories`
    categories.columns = category_colnames

    for column in categories.columns:
        # set each value to be the last character of the string
        # and convert column from string to numeric
        categories[column] = categories[column].apply(lambda x: x[-1]).astype('int')
        
    # drop the original categories column from `df`
    df = df.drop('categories', axis=1)

    # concatenate the original dataframe with the new `categories` dataframe
    for col in categories.columns:
        df[col] = categories[col]

    # drop duplicates and return dataframe
    return df.drop_duplicates() 


def save_data(df, database_filename):
    '''
    Function that saves DataFrame to SQL.
    Inputs:
        df: DataFrame to be saved
        database_filename: filename of the SQL database 
    Output: 
        None
    '''
    engine = create_engine('sqlite:///messages.db')
    df.to_sql(database_filename, engine, if_exists='replace', index=False)


def main():
    '''
    Function that loads, cleans and saves dataset in SQL DB.
    Inputs: 
        messages_filepath, categories_filepath: filepath of the cvs files 
        database_filepath:  filename of the SQL database
    Output:
        None
    '''
    if len(sys.argv) == 4:

        messages_filepath, categories_filepath, database_filepath = sys.argv[1:]

        print('Loading data...\n    MESSAGES: {}\n    CATEGORIES: {}'
              .format(messages_filepath, categories_filepath))
        df = load_data(messages_filepath, categories_filepath)

        print('Cleaning data...')
        df = clean_data(df)
        
        print('Saving data...\n    DATABASE: {}'.format(database_filepath))
        save_data(df, database_filepath)
        
        print('Cleaned data saved to database!')
    
    else:
        print('Please provide the filepaths of the messages and categories '\
              'datasets as the first and second argument respectively, as '\
              'well as the filepath of the database to save the cleaned data '\
              'to as the third argument. \n\nExample: python process_data.py '\
              'disaster_messages.csv disaster_categories.csv '\
              'DisasterResponse.db')


if __name__ == '__main__':
    main()